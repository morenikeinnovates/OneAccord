...
export default function SessionPage() {
  ...
  const [currentStep, setCurrentStep] = useState<'mystory' | 'yourstory' | 'middle'>('mystory');
    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white shadow">
        <div className="max-w-2xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <span className="text-gray-400">
            {currentStep === 'mystory' ? '1/3' : currentStep === 'yourstory' ? '2/3' : '3/3'}
          </span>
        </div>
      </nav>
      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow p-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-4">{sessionItem.title}</h2>

          <div className="mb-6">
            <p className="text-gray-600 mb-4 whitespace-pre-wrap">{sessionItem.why}</p>
            {sessionItem.scripture && (
              <div className="bg-blue-50 border-l-4 border-blue-500 p-4 italic text-gray-700">
                ...
                <p className="text-sm font-semibold mt-2">— {sessionItem.scripture.ref}</p>
              </div>
            )}
          </div>
          ...
          >
            {saving ? 'Saving...' : currentStep === 'middle' ? 'Complete Session' : 'Next Step'}
          </button>
        </div>
      </main>
    </div>
